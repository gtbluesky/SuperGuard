...
import com.gtbluesky.guard.res.decoder.StringBlock;
...
public class RawARSCDecoder {
    ...
    private HashMap<Integer, Set<String>> mExistTypeNames;
...
//    }
//
//    public static Set<String> getAllTypeSpecNameStrings() {
//        Set<String> set = new HashSet<>();
//        for (Set<String> s : mExistTypeNames.values()) {
//            set.addAll(s);
//        }
//        return set;
//    }
    ...
    private void readConfigFlags() throws IOException, AndrolibException {
        ...
        if (exceedingSize > 0) {
            ...
            if (exceedingBI.equals(BigInteger.ZERO)) {
                System.out.println(String.format("Config flags size > %d, but exceeding bytes are all zero, so it should be ok.",
                        KNOWN_CONFIG_BYTES
                ));
            } else {
                System.out.println(String.format("Config flags size > %d. Exceeding bytes: 0x%X.",
                        KNOWN_CONFIG_BYTES,
                        exceedingBI
                ));
            }
