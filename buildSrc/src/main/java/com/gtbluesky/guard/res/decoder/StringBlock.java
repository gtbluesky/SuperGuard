    ...
    public static int writeSpecNameStringBlock(
        ...
        for (String s : specNames.keySet()) {
            ...
            if (isUTF8) {
                ...
                stringBytes[offset++] = (byte) bytes.length;
                stringBytes[offset++] = (byte) bytes.length;
                totalSize += 2;
                ...
                writeShort(stringBytes, offset, (short) bytes.length);
                offset += 2;
                totalSize += 2;
                System.arraycopy(bytes, 0, stringBytes, offset, bytes.length);
                offset += bytes.length;
    ...
    public static int writeTableNameStringBlock(
        ...
        for (i = 0; i < stringCount; i++) {
            ...
            if (tableProguardMap.get(i) == null) {
                ...
                if (block.m_isUTF8) {
                    byte[] bytes = name.getBytes(Charset.forName("UTF-8"));
                    strings[offset++] = (byte) bytes.length;
                    strings[offset++] = (byte) bytes.length;
                    totalSize += 2;
                    System.arraycopy(bytes, 0, strings, offset, bytes.length);
                    offset += bytes.length;
                    strings[offset++] = NULL;
                    totalSize += bytes.length + 1;
                } else {
                    byte[] bytes = name.getBytes(Charset.forName("UTF-16LE"));
                    writeShort(strings, offset, (short) bytes.length);
                    offset += 2;
                    totalSize += 2;
                    System.arraycopy(bytes, 0, strings, offset, bytes.length);
                    offset += bytes.length;
